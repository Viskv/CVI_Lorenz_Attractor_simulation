...
/*----------------------------------------------------------------------------*/
/*    Copyright (c) National Instruments 2000-2020.  All Rights Reserved.     */
/*----------------------------------------------------------------------------*/
/*                                                                            */
/* Title:       ExcelReport.c                                                 */
...
/*				(Excel.fp).  This generated driver must be present to use */
/*				this library.												  */
/*                                                                            */
/*============================================================================*/
									  ...
									  CAObjHandle *workbookHandle)
{
	...
	__caErrChk (Excel_Workbooks_Open (WorkbooksHandle, ExcelRpt_GetErrorInfo(), fileName, CA_DEFAULT_VAL, CA_DEFAULT_VAL,
									  ...
									  enum ExREnum_ExFileFormat format)
{
    ...
    if(fileName != NULL)
    {
        ...
        __caErrChk (Excel_Workbook__SaveAs (workbookHandle, ExcelRpt_GetErrorInfo(), FileNameV,
...
HRESULT CVIFUNC ExcelRpt_InsertRow (CAObjHandle worksheetHandle, int insertBeforeRowIndex, int numRows)
{
    ...
    for(i = 0; i < numRows; i++)
    {
        __caErrChk (Excel_RangeInsert (rowHandle, ExcelRpt_GetErrorInfo(), CA_DEFAULT_VAL, CA_DEFAULT_VAL, NULL));
    }
...
HRESULT CVIFUNC ExcelRpt_InsertColumn (CAObjHandle worksheetHandle, int insertBeforeColumnIndex, int numColumns)
{
    ...
    {
        __caErrChk (Excel_RangeInsert (columnHandle, ExcelRpt_GetErrorInfo(), CA_DEFAULT_VAL, CA_DEFAULT_VAL, NULL));
    }
							  ...
							  int matchCase, int matchByte, char foundRange[])
{
    ...
    __caErrChk (Excel_RangeFind (worksheetRangeHandle, ExcelRpt_GetErrorInfo(), DataV, LookforV, CA_VariantInt(lookIn),
    							 ...
    							 CA_VariantBool(matchByte ? VTRUE : VFALSE),
    							 CA_DEFAULT_VAL, &resultHandle));
							  ...
							  enum ExREnum_ExYesNo header)
{
    ...
    __caErrChk (Excel_Range_Sort (rangeHandle, ExcelRpt_GetErrorInfo(), ObjkeyV1,
    							  ...
    							  CA_DEFAULT_VAL,
								  (enum ExcelEnum_XlSortOrientation)sortOrientation, ExcelConst_xlPinYin,
								  ExcelConst_xlSortNormal, ExcelConst_xlSortNormal, ExcelConst_xlSortNormal, NULL));
									 ...
									 enum ExREnum_ExInsertShiftDirection shiftDirection)
{
    ...
    __caErrChk (Excel_RangeInsert (rangeHandle, ExcelRpt_GetErrorInfo(), CA_VariantInt(shiftDirection), CA_DEFAULT_VAL, &ResultV));
								   ...
								   enum ExREnum_ExDataType dataType, size_t dim1, size_t dim2, const void *data)
{
    ...
    __caErrChk (Excel_SetProperty (rangeHandle, ExcelRpt_GetErrorInfo(), Excel_RangeValue2, CAVT_VARIANT, ArrayV));
								  ...
								  enum ExREnum_ExDataType dataType, void *data)
{
    ...
    __caErrChk (Excel_GetProperty (rangeHandle, ExcelRpt_GetErrorInfo(), Excel_RangeValue2, CAVT_VARIANT, &RangeV));
										 ...
										 enum ExREnum_ExDataType *dataType)
{
    ...
    __caErrChk (Excel_GetProperty (cellHandle, ExcelRpt_GetErrorInfo(), Excel_RangeValue2, CAVT_VARIANT, &CellV));
...
HRESULT CVIFUNC ExcelRpt_GetCellValueStringLen(CAObjHandle worksheetHandle, const char *cellRange, int *length)
{
    ...
    __caErrChk (Excel_GetProperty (cellHandle, ExcelRpt_GetErrorInfo(), Excel_RangeValue2, CAVT_VARIANT, &ValueV));
									  ...
									  enum ExREnum_ExDataType dataType, ...)
{
	...
	__caErrChk (Excel_SetProperty (cellHandle, ExcelRpt_GetErrorInfo(), Excel_RangeValue2, CAVT_VARIANT, SetValueV));
